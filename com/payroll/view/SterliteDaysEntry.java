...
public class SterliteDaysEntry extends BaseClass implements ActionListener 
{
	...
	//	getContentPane().add(label_2);
		...
		lblDispatchEntry = new JLabel("Sterile Days Entry");
		...
		//////////////////////////////////////////////////////////////////////
		String[] crDrColName = {"Sno","Employee Name","ESIC No","PF No.","Code","Sterile Allow","Sterile Days",""};
		String cdDrData[][] = {};
		...
		AttdTable.getColumnModel().getColumn(5).setPreferredWidth(100);//Sterlite allow
		AttdTable.getColumnModel().getColumn(6).setPreferredWidth(125);//Sterlite Days
			...
			public void keyPressed(KeyEvent evt) {
				...
				if (evt.getKeyCode() == KeyEvent.VK_ENTER) 
				{
					...
					if (column == 6) 
					{
						...
						if(row<totRow-1)
						{
							AttdTable.changeSelection(row+1, 6, false, false);
							AttdTable.editCellAt(row+1, 6);
						}
						else
						{
							AttdTable.changeSelection(row, 6, false, false);
	...
	public ArrayList Update() 
	{
		...
		try {
			   ...
			   for (int i = 0; i < s; i++) 
			   {
				   ...
				   emp.setStair_alw(setDoubleNumber(col.get(5).toString()));
				   emp.setStair_days(setIntNumber(col.get(6).toString()));
	...
	public void fillTable()
	{
		...
		Vector<?> v = pdao.getSterliteDaysList(loginDt.getDepo_code(),loginDt.getCmp_code(), loginDt.getFin_year(), mdto.getMnthcode(),1); 
		int size = v.size();
		if(size>0)
		{
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if(e.getActionCommand().equalsIgnoreCase("Submit"))
		{
			...
			if(ans==1)
			{
				h = pdao.lockEntry(loginDt.getLogin_id(),loginDt.getLogin_name(),loginDt.getDepo_code(),loginDt.getCmp_code(),mdto.getMnthcode(),1);
				if(h>0)
				{
					alertMessage(SterliteDaysEntry.this, "Record Locked Sucessfully!!!!");
					//fillTable();
				}
			}
...
//				new AttendanceList(loginDt.getBrnnm(), loginDt.getDrvnm(),l, mdto.getMnthname());
				new EsicList(loginDt.getDepo_code(), loginDt.getCmp_code(), loginDt.getFin_year(), mdto.getMnthcode(), loginDt.getBrnnm(), loginDt.getDrvnm(), mdto.getMnthname(), 2, 4,0);
		}		
	}
