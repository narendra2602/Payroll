...
public class AttendanceEntryFinal extends BaseClass implements ActionListener 
{
...
//		getContentPane().add(label_2);
	...
	public void fillTable()
	{
		...
		for(int i =0;i<size;i++)
		{
		 ...
		 lock = ((EmptranDto) c.get(9)).getAtten_lock();
//		month.setSelectedIndex(getIndex(mnthcode, month, 5));
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if(e.getActionCommand().equalsIgnoreCase("Final"))
		{
			int ans=confirmationDialong();
			int h=0;
			if(ans==1)
			{
				h = pdao.lockAttendance(loginDt.getFin_year(),loginDt.getDepo_code(),loginDt.getCmp_code(),mdto.getMnthcode(),1);
				if(h>0)
				{
					alertMessage(AttendanceEntryFinal.this, "Attendance Record Locked Sucessfully!!!!");
					fillTable();
				}
			}
