...
public class LTACalculation extends BaseClass implements ActionListener 
{
...
//		getContentPane().add(label_2);
		...
		lblDispatchEntry = new JLabel("LTA/Medical Calculation");
		...
		//////////////////////////////////////////////////////////////////////
		String[] crDrColName = {"Sno","Employee Name","ESIC No","PF No.","Code","LTA ","Medical","","",""};
		String cdDrData[][] = {};
		...
		AttdTable.getColumnModel().getColumn(8).setMinWidth(0); // hidden emptranDto
		AttdTable.getColumnModel().getColumn(8).setMaxWidth(0);

		AttdTable.getColumnModel().getColumn(9).setPreferredWidth(0);
		AttdTable.getColumnModel().getColumn(9).setMinWidth(0); // hidden lock
	...
	public ArrayList Update() 
	{
		...
		try {
			   ...
			   for (int i = 0; i < s; i++) 
			   {
				   ...
				   emp.setLta_value(setDoubleNumber(col.get(5).toString()));
				   emp.setMedical_value(setDoubleNumber(col.get(6).toString()));
	...
	public void fillTable()
	{
		...
		int mncode=0;
		Vector<?> v = pdao.getLTAList(loginDt.getDepo_code(),loginDt.getCmp_code(), loginDt.getFin_year(), mdto.getMnthcode(),5); 
		int size = 0;
		  if(v!=null)
				size=v.size();
		if(size>0)
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if(e.getActionCommand().equalsIgnoreCase("Submit"))
		{
			...
			if(ans==1)
			{
				h = pdao.lockEntry(loginDt.getLogin_id(),loginDt.getLogin_name(),loginDt.getDepo_code(),loginDt.getCmp_code(),mdto.getMnthcode(),5);
				if(h>0)
				{
					alertMessage(LTACalculation.this, "Record Locked Sucessfully!!!!");
					fillTable();
				}
			}
		...
		{
			new EsicList(loginDt.getDepo_code(), loginDt.getCmp_code(), loginDt.getFin_year(), mdto.getMnthcode(), loginDt.getBrnnm(), loginDt.getDrvnm(), mdto.getMnthname(), 2, 12,0);
		}		
	}
	...
	{
		System.out.println(loginDt.getMno());
