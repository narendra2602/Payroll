...
public class ProfessionalTaxEntry extends BaseClass implements ActionListener 
{
...
//		getContentPane().add(label_2);
		...
		lblDispatchEntry = new JLabel("Professional Tax Entry");
		...
		//////////////////////////////////////////////////////////////////////
		String[] crDrColName = {"Sno","Employee Name","ESIC No","PF No.","Code","P.Tax Amount","",""};
		String cdDrData[][] = {};
		...
		AttdTable.getColumnModel().getColumn(1).setPreferredWidth(320);//name
		AttdTable.getColumnModel().getColumn(2).setPreferredWidth(150);//ESIC no
		AttdTable.getColumnModel().getColumn(3).setPreferredWidth(150);//PF No
		AttdTable.getColumnModel().getColumn(4).setPreferredWidth(100);//code
		AttdTable.getColumnModel().getColumn(5).setPreferredWidth(120);//Misc value

		AttdTable.getColumnModel().getColumn(6).setPreferredWidth(0);
		AttdTable.getColumnModel().getColumn(6).setMinWidth(0); // hidden emptranDto
		AttdTable.getColumnModel().getColumn(6).setMaxWidth(0);
			...
			public void keyPressed(KeyEvent evt) {
				...
				if (evt.getKeyCode() == KeyEvent.VK_ENTER) 
				{
					...
					if (column == 5) 
					{
						AttdTable.changeSelection(row, 2, false, false);
						AttdTable.editCellAt(row,2);
						double val = setDoubleNumber(AttdTable.getValueAt(row, 5).toString());
						AttdTable.setValueAt(val,row, 5);
						if(row<totRow-1)
						{
	...
	public ArrayList Update() 
	{
		...
		try {
			   ...
			   for (int i = 0; i < s; i++) 
			   {
				   ...
				   emp.setProf_tax(setDoubleNumber(col.get(5).toString()));
	...
	public void fillTable()
	{
		...
		Vector<?> v = pdao.getSterliteDaysList(loginDt.getDepo_code(),loginDt.getCmp_code(), loginDt.getFin_year(), mdto.getMnthcode(),7); 
		int size = v.size();
		if(size>0)
		{
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if(e.getActionCommand().equalsIgnoreCase("Submit"))
		{
			...
			if(ans==1)
			{
				h = pdao.lockEntry(loginDt.getLogin_id(),loginDt.getLogin_name(),loginDt.getDepo_code(),loginDt.getCmp_code(),mdto.getMnthcode(),7);
				if(h>0)
				{
					alertMessage(ProfessionalTaxEntry.this, "Record Locked Sucessfully!!!!");
					//fillTable();
				}
			}
		...
		{
			new EsicList(loginDt.getDepo_code(), loginDt.getCmp_code(), loginDt.getFin_year(), mdto.getMnthcode(), loginDt.getBrnnm(), loginDt.getDrvnm(), mdto.getMnthname(), 2, 11,0);
		}		
	}
